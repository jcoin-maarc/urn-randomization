...
from bokeh.models import ColumnDataSource, Paragraph, Legend, CDSView, GroupFilter, IndexFilter, Select, HoverTool, FactorRange, PrintfTickFormatter
...
def plt_factor_treatment_assignments(study):
	...
	pdf_factors = pdf_urns.assign(**dict([('pc_trt_{0}'.format(trt).replace("-", ""),
	                                       pdf_urns['trt_{0}'.format(trt)] * 100 /
	                                       pdf_urns[[col for col in pdf_urns.columns if col.startswith('trt_')]]
	...
	lst_vbar = p.vbar_stack([trt for trt in study.treatments],
	             ...
	             source=src,
	             view=view)
	legend = Legend(items=[(trt, [plt_vbar]) for (trt, plt_vbar) in zip(study.treatments,
	                                                                    lst_vbar)],
	                location=(0,30),
	                title='Treatments')
	...
	callback = CustomJS(args=dict(select=select_factor, src=src, filter=filter,
	                              factors=study.factors, plot=p), code='''
